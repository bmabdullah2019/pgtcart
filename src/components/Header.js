...
export default function Header({ config, contact, categories, pages }) {
  ...
  const handleLogout = (e) => {
    ...
    if (typeof window !== "undefined") {
      ...
      window.location.href = `${BACKEND_URL}/customer/logout?redirect=${encodeURIComponent(window.location.origin + "/")}`;
    }
  };
    ...
    <header className="w-full font-sans sticky top-0 z-50 shadow-md bg-white">
      ...
      <div className="hidden md:block bg-white border-b border-gray-100">
        <div className={`${headerContainerClass} flex items-center justify-between gap-2 md:gap-4 py-2 md:py-4`}>
          ...
          <div className="flex items-center gap-2 md:gap-3 flex-1 justify-end min-w-0">
            {customer ? (
              <div className="flex items-center gap-2">
                <div className="text-[11px] md:text-xs font-bold text-gray-800 bg-amber-50 border border-[#ffd300]/30 px-3 py-1.5 rounded-full flex items-center gap-1.5">
                  <svg className="w-3.5 h-3.5 text-[#c29900] fill-none stroke-current" strokeWidth="2" viewBox="0 0 24 24">
                    <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2M12 7a4 4 0 1 0 0-8 4 4 0 0 0 0 8z" />
                  </svg>
                  <span className="truncate max-w-[80px] md:max-w-[120px]">{customer.name}</span>
                </div>
                <button
                  onClick={handleLogout}
                  className="text-[10px] md:text-xs font-extrabold text-rose-600 hover:text-rose-700 hover:bg-rose-50 border border-rose-100 hover:border-rose-200 px-2.5 py-1.5 rounded-full transition-colors cursor-pointer"
                >
                  Logout
                </button>
              </div>
      ...
      <div className="bg-[#ffd300] text-black text-[12px] font-semibold py-2.5 px-4 flex justify-between items-center md:hidden">
        ...
        <div className="flex items-center gap-3 text-[11px]">
          {customer ? (
            <>
              <span className="font-bold text-gray-800">Hello, {customer.name}</span>
              <span className="text-black/20">|</span>
              <button onClick={handleLogout} className="hover:underline font-bold text-rose-600 cursor-pointer">
                Logout
              </button>
            </>
          ) : (
            <>
              <Link href="/customer/register" className="hover:underline">
                Create an Account
              </Link>
              <span className="text-black/20">|</span>
              ...
              {customer ? (
                <div className="flex flex-col gap-2 p-1">
                  <div className="flex items-center gap-2 text-gray-800">
                    ...
                    <span>Hello, <strong className="text-black">{customer.name}</strong></span>
                  </div>
                  <button
                    onClick={(e) => {
                    ...
                    className="w-full text-left text-rose-600 font-bold mt-1 flex items-center gap-2 cursor-pointer"
                  >
                    <svg className="w-4 h-4 fill-none stroke-current text-rose-500" strokeWidth="2" viewBox="0 0 24 24">
